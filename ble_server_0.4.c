...
static void *scan_le(void *client_sock)
{
	...
	scan_params_cp.type 			= 0x00; //00 Passive
...
{
    int socket_desc,c,temp,option_value;
		char ret = '\0';
    ...
    for(int id;id < MAX_CLIENTS; ){
      while (connected_clients[current_client] = accept(socket_desc, (struct sockaddr *)&client, (socklen_t*)&c))
      {
          puts("Connection accepted");
  				printf("\n Connected to (%s : %d)\n",inet_ntoa(client.sin_addr),ntohs(client.sin_port));
          if( pthread_create( &thread_id[id] , NULL ,  scan_le , (void*) &connected_clients[current_client]) < 0)
          {
          ...
          puts("Handler assigned");
					if (id >= MAX_CLIENTS) {
						printf("clients have been overflowed\n" );
						printf("Clearing up old connections...\n" );
						id = 0;
						current_client = 0;
					}
      }
      if (connected_clients[current_client] < 0)
      {
